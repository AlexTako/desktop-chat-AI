...
class ChatCache:
    ...
    def create_tables(self):
        ...
        # Создание таблицы для хранения API ключа и PIN-кода
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS auth_data (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                api_key TEXT NOT NULL,               -- API ключ OpenRouter
                pin TEXT NOT NULL,                   -- PIN-код для входа
    ...
    def save_auth_data(self, api_key, pin):
        ...
        Args:
            ...
            pin (str): 4-значный PIN-код для входа
        ...
        try:
            ...
            # Удаляем предыдущие данные аутентификации, если они есть
            cursor.execute('DELETE FROM auth_data')
            
            # Вставляем новые данные
            cursor.execute('''
                INSERT INTO auth_data (api_key, pin, created_at)
                VALUES (?, ?, ?)
            ''', (api_key, pin, datetime.now()))
    ...
    def check_pin(self, pin):
        ...
        try:
            ...
            cursor.execute('SELECT pin FROM auth_data LIMIT 1')
            result = cursor.fetchone()
            
            # Если в базе нет записей с PIN-кодом
            if not result:
                return False
    ...
    def get_api_key(self):
        ...
        Returns:
            str or None: API ключ, если он существует, иначе None
        """
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
    ...
    def has_auth_data(self):
        """
        Проверка наличия данных аутентификации в базе.
        
        Returns:
            bool: True если данные аутентификации существуют, иначе False
        """
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
