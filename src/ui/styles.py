...
class AppStyles:
    ...
    # Настройки IconButton для отправки сообщения (без параметра text)
    SEND_ICON_BUTTON = {
        "icon": ft.Icons.SEND,               # Иконка отправки сообщения
        "style": ft.ButtonStyle(             # Стиль оформления кнопки
            color=ft.Colors.WHITE,           # Цвет текста кнопки
    ...
    # Настройки IconButton для очистки истории (без параметра text)
    CLEAR_ICON_BUTTON = {
        "icon": ft.Icons.DELETE,             # Иконка удаления
        "style": ft.ButtonStyle(             # Стиль оформления кнопки
            color=ft.Colors.WHITE,           # Цвет текста
    ...
    ANALYTICS_BUTTON = {
        ...
        "tooltip": "Показать статистику",    # Всплывающая подсказка
        "width": 130,                        # Ширина кнопки
        "height": 40,                        # Высота кнопки
    }

    # Настройки кнопки экспорта истории
    EXPORT_BUTTON = {
        "text": "Экспорт",                   # Текст на кнопке
        "icon": ft.Icons.DOWNLOAD,           # Иконка загрузки/экспорта
        "style": ft.ButtonStyle(             # Стиль оформления кнопки
            color=ft.Colors.WHITE,           # Цвет текста
            bgcolor=ft.Colors.BLUE_700,      # Цвет фона
            padding=10,                      # Внутренние отступы
        ),
        "tooltip": "Экспорт истории",        # Всплывающая подсказка
        "width": 130,                        # Ширина кнопки
        "height": 40,                        # Высота кнопки
    }

    # Настройки IconButton для экспорта истории (без параметра text)
    EXPORT_ICON_BUTTON = {
        "icon": ft.Icons.DOWNLOAD,           # Иконка загрузки/экспорта
        "style": ft.ButtonStyle(             # Стиль оформления кнопки
            color=ft.Colors.WHITE,           # Цвет текста
    ...
    # Настройки строки с элементами управления (дублирует CONTROL_BUTTONS_ROW для совместимости)
    CONTROLS_ROW = {
        "spacing": 20,                            # Отступ между кнопками
        "alignment": ft.MainAxisAlignment.CENTER,  # Выравнивание кнопок по центру
    }
    ...
    @staticmethod
    def create_button(button_type: str, style_name: str, **overrides):
        """
        Создает кнопку нужного типа с применением соответствующих стилей.
        
        Args:
            button_type (str): Тип кнопки - 'icon' для IconButton или 'text' для ElevatedButton
            style_name (str): Имя стиля из AppStyles (без префикса и постфикса)
            **overrides: Переопределения для стандартных параметров
            
        Returns:
            ft.Control: Созданная кнопка с примененными стилями
            
        Examples:
            # Создание кнопки отправки как IconButton
            send_button = AppStyles.create_button('icon', 'SEND', on_click=send_callback)
            
            # Создание кнопки сохранения как обычной кнопки
            save_button = AppStyles.create_button('text', 'SAVE', on_click=save_callback)
        """
        if button_type.lower() == 'icon':
            # Для IconButton используем стиль с постфиксом _ICON_BUTTON
            style_attr = f"{style_name}_ICON_BUTTON"
            if hasattr(AppStyles, style_attr):
                props = {**getattr(AppStyles, style_attr), **overrides}
                return ft.IconButton(**props)
            else:
                # Если специфичного стиля нет, берем обычный и удаляем text
                props = {**getattr(AppStyles, style_name), **overrides}
                if "text" in props:
                    props.pop("text")
                return ft.IconButton(**props)
        else:
            # Для обычных кнопок используем стандартный стиль
            props = {**getattr(AppStyles, f"{style_name}_BUTTON"), **overrides}
            return ft.ElevatedButton(**props)
